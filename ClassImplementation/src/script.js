...
const mercuryRadius = .30 * radiusMod
...
const defaultTrailMaterial = new THREE.LineBasicMaterial({color: 0xffffff});
...
class Body
{
    ...
    albedo = .1; // visible light reflected
    emissivity = .9; // infrared light emitted
    trailMaterial = defaultTrailMaterial;
...
class Star extends Body
{
    ...
    constructor(name, x, y, z, vx, vy, vz, mesh, mass, radius, rotation, luminosity)
    {
...
const createPlanetMesh = (radius, textureSource) =>
{
...
// Mercury
const mercury = createPlanetMesh(mercuryRadius, 'mercury.jpg')
scene.add(mercury)

// Earth
const earth = createPlanetMesh(earthRadius, 'earth.jpg')
scene.add(earth)
...
const earthObject = new Body('Earth', AU, 0, 0, 0, 0, -29783, earth, earthMass, earthRadius, earthRotation);
const mercuryObject = new Body('Mercury', .4 * AU, 0, 0, 0, 0, -47000, mercury, mercuryMass, mercuryRadius, mercuryRotation);
...
const resetSimulation = () =>
{
    for(const planet of planets)
    {
        planet.x = planet.defaultObject.x
        planet.y = planet.defaultObject.y
        planet.z = planet.defaultObject.z
        planet.vx = planet.defaultObject.vx
        planet.vy = planet.defaultObject.vy
        planet.vz = planet.defaultObject.vz
        planet.mesh.position.x = planet.x / parameters.scaleFactor
        planet.mesh.position.y = planet.y / parameters.scaleFactor
        planet.mesh.position.z = planet.z / parameters.scaleFactor
...
let mercuryTempText = document.getElementById("mercuryTemp")
...
const tick = () =>
{
    ...
    let mercuryTemp = calculateTemperature(mercuryObject, sunObject);

    earthTempText.innerText = "Earth: \t"+ parseFloat(earthTemp).toFixed(2) +'\n'
    mercuryTempText.innerText = "Mercury: "+ parseFloat(mercuryTemp).toFixed(2)
