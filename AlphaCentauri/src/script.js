 ...
 * System Center Mesh
 */
const centerMesh = new THREE.Mesh(new THREE.SphereGeometry(.1, 8, 8), new THREE.MeshStandardMaterial({color:0xffffff}))
 ...
 */
// Alpha Centauri A
const alphaCentauriA = createStarMesh(alphaCentauriARadius, 'sun.jpg', '#ffeb0a', .75)
 ...
 */
const alphaCentauriAObject = new Star('Alpha Centauri A', -10.9 * AU/2, 0, 0, 0, 0, 5000, alphaCentauriA, alphaCentauriAMass, alphaCentauriARadius, alphaCentauriARotation, alphaCentauriALuminosity, alphaCentauriAPointLightIntensity);
const alphaCentauriBObject = new Star('Alpha Centauri B', 12.8 * AU/2, 0, 0, 0, 0, -5000, alphaCentauriB, alphaCentauriBMass, alphaCentauriBRadius, alphaCentauriBRotation, alphaCentauriBLuminosity, alphaCentauriBPointLightIntensity);
const proximaCentauriObject = new Star('Proxima Centauri', 2500 * AU, 0, 0, 0, 0, -11000, proximaCentauri, proximaCentauriMass, proximaCentauriRadius, proximaCentauriRotation, proximaCentauriLuminosity, proximaCentauriPointLightIntensity);
const proximaCentauriBObject = new Planet('Proxima Centauri B', 2500 * AU + .5 * AU, 0, 0, 0, 0, -11000 - 12500, proximaCentauriB, proximaCentauriBMass, proximaCentauriBRadius, proximaCentauriBRotation, proximaCentauriBAlbedo, proximaCentauriBEmissivity);
const proximaCentauriCOCject = new Planet('Proxima Centauri C', 2500 * AU + 1.49 * AU, 0, 0, 0, 0, -11000 - 7500, proximaCentauriC, proximaCentauriCMass, proximaCentauriCRadius, proximaCentauriCRotation, proximaCentauriCAlbedo, proximaCentauriCEmissivity);
...
const setupGui = () =>
{
    gui.add(parameters, 'timeStep').min(100).max(250000).step(10).name('Time Step')
    gui.add(parameters, 'scaleFactor').min(defaultScaleFactor / 50).max(5 * defaultScaleFactor).step(10)
 ...
 */
 let proximaBTempText = document.getElementById("proximaBTemp")
 let proximaCTempText = document.getElementById("proximaCTemp")
...
const tick = () =>
{
    ...
    let proximaBTemp = calculateTemperature(proximaCentauriBObject, proximaCentauriObject);
    let proximaCTemp = calculateTemperature(proximaCentauriCOCject, proximaCentauriObject);
    proximaBTempText.innerText = "Proxima B: "+ parseFloat(proximaBTemp).toFixed(2) +'\n'
    proximaCTempText.innerText = "Proxima C: "+ parseFloat(proximaCTemp).toFixed(2)
