...
const jupiterRadius = .9 * radiusMod
const saturnRadius = .85 * radiusMod
const uranusRadius = .8 * radiusMod
const neptuneRadius = .75 * radiusMod
const plutoRadius = .2 * radiusMod
...
const updateTempText = () =>
{
    for(var body of planets)
    {
        ...
        {
            let temp = calculateTemperature(body, sunObject)
...
const setupGui = () =>
{
    gui.add(parameters, 'timeStep').min(100).max(200000).step(10).name('Time Step')
    ...
    gui.add(skyBox, "visible").name("Sky Box Visible")
    gui.add(parameters, "systemVelocityY").min(-20000).max(20000).step(500).name("System Velocity Y").onChange(updateSystemVelocity)
